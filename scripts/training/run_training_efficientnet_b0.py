"""
Training Entry Point -- EfficientNet-B0
DeepSceneLoc -- Semester 2, Week 8

Authors:
    Krishan Yadav  (Model Architecture Lead)
    Anuj Kondawar  (Pipeline & Training Lead)

This script is the entry point for training the EfficientNet-B0
advanced model. It wraps:
  - src.models.model_advanced   (EfficientNet definition)
  - src.models.train_advanced   (AdvancedTrainer, EfficientNetTrainConfig)
...
Usage (activate venv first):
    ...
    python run_training_efficientnet_b0.py --batch 128        # 64/GPU on T4 x2
    python run_training_efficientnet_b0.py --no-multi-gpu     # force single-GPU

    # Max-accuracy full fine-tune on T4 x2 (LLRD + SWA)
    python run_training_efficientnet_b0.py --batch 128 --epochs 45 \\
        --full-finetune --swa
    ...
    python run_training_efficientnet_b0.py --dry-run

    # Evaluate a saved checkpoint
    python run_training_efficientnet_b0.py --eval-only \\
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train EfficientNet-B0 on the DeepSceneLoc dataset"
    )
    # Data
    ...
    p.add_argument("--freeze-blocks", type=int,  default=None,
                   help="Number of backbone blocks to freeze")
    ...
    p.add_argument("--full-finetune", action="store_true",
                   help="Unfreeze the whole backbone (freeze-blocks=0) + enable LLRD")
    p.add_argument("--llrd", type=float, default=None,
                   help="Layer-wise LR decay factor (e.g. 0.75). Implied by --full-finetune")
...
def build_config(args):
    """
    Create the EfficientNetTrainConfig, overriding defaults with any
    CLI arguments the user explicitly provided.
    """
    ...
    if args.full_finetune:
        cfg.llrd_decay = args.llrd if args.llrd is not None else 0.75
...
def main():
    ...
    print("\n" + "=" * 65)
    print(f"  DeepSceneLoc -- EfficientNet-B0 Training (Modern Pipeline)")
    ...
    if args.full_finetune:
        freeze_blocks = 0               # unfreeze entire backbone
